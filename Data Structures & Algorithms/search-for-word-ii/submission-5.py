    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for y in range(ROW):
            for x in range(COL):
                solve(x, y)
        return list(res)
        #WCRT: O(M * N * 4^W) | Space: O(N * M + S) extra space and O(X) for output list where M is row and N is col and W is max(words) S is the total number of characters in words and X is the length of words.